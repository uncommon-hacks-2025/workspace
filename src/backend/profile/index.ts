...
export async function createProfile(fullName: string, dateOfBirth: Date, history: MedicalHistory[]): Promise<Profile | null> {
    ...
    if (user === null) {
        ...
        const medicalConditions = await prisma.medicalCondition.createMany({
            data: await Promise.all(history.map(async (condition) => ({
